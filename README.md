# CSA Exam Drift Detector

**Scope Prefix:** `x_csa_exam_drift`
**Repository:** `vladarchitectservicenow-oss/sn_csa_exam_drift_detector`
**License:** MIT
**Author:** Vladimir Kapustin

## Overview

CSA Exam Drift Detector is an enterprise-grade ServiceNow scoped application designed to solve critical platform challenges that organizations face during upgrades, migrations, and operational governance. Monitors and detects drift between a ServiceNow instance configuration and the CSA (Certified System Administrator) exam blueprint, highlighting practice areas for certification prep. This application was built specifically for the Australia-era ServiceNow platform, leveraging the latest APIs, table schemas, and automation frameworks to deliver a seamless, native experience within any ServiceNow instance.
...
- **CSAExamDriftDetectorScanner** — Executes regex matching across configured tables. Exposes `scan()` and `scanIncremental(sinceDate)`. Returns a result object containing findings, statistics, and execution time.
- **CSAExamDriftDetectorRuleEngine** — Loads deprecation rules from the application table. Exposes `evaluate(scriptText)` and `getReplacement(ruleId)`. Supports custom rule injection for enterprise-specific deprecations.
- **CSAExamDriftDetectorReportGenerator** — Transforms finding records into HTML, JSON, or PDF. Exposes `generateHTML(scanRunId)`, `generateJSON(scanRunId)`, and `generatePDF(scanRunId)`.
...
## Overview
sn_csa_exam_drift_detector is a production-grade ServiceNow scoped application developed by Vladimir Kapustin under AGPL-3.0.

## Architecture
```mermaid
graph TD
    SN[ServiceNow Instance] -->|REST| sn_csa_exam_drift_detector
    sn_csa_exam_drift_detector -->|Store| DB[x_sn_csa_exam_drift_detector_tables]
    sn_csa_exam_drift_detector -->|Output| Report[Reports MD/JSON/CSV]
...
## Installation
```bash
git clone https://github.com/vladarchitectservicenow-oss/sn_csa_exam_drift_detector.git
cd sn_csa_exam_drift_detector
...
## ROI Analysis
| Metric | Manual Process | With sn_csa_exam_drift_detector |
|--------|---------------|-------------|
...
# Run scan
POST /api/x_sn_csa_exam_drift_detector/scan
...
See `Validation/TEST CASES/sn_csa_exam_drift_detector/test_suite_SOP.md`
...
## Support
- GitHub Issues: https://github.com/vladarchitectservicenow-oss/sn_csa_exam_drift_detector/issues
- ServiceNow Community: Tag `sn_csa_exam_drift_detector`
